...
from basic_vae.generator import Generator
...
def train_epoch(sess, encoder, generator, data_loader):
    ...
    for it in range(data_loader.num_batch):
        ...
        _, r_loss = generator.train_step(sess, batch)
...
def main():
    ...
    # configuration
    config = configs.CONFIG_MAP['t_64'] # t_20 t_8
    ...
    log_fn = '../save/basic_vae_' + time_str + '.log'
    ...
    with graph.as_default():
        ...
        for total_epoch in range(TOTAL_EPOCH):
            kl_loss, r_loss = train_epoch(sess, encoder, generator, gen_data_loader)
            if total_epoch % 5 == 0:
